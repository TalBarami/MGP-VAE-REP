...
def plot_training_images(video1, video2, dec_v1, dec_v2, X1_v1, X1_v2, epoch, decoder_test):
    ...
    axs[1, 0].imshow(plot_image(dec_v1.squeeze(0), nrow=NUM_FRAMES, normalize=True))
    axs[1, 0].set_title('recon_v1', fontsize=6)
    axs[1, 0].axis('off')

    axs[1, 1].imshow(plot_image(dec_v2.squeeze(0), nrow=NUM_FRAMES, normalize=True))
    axs[1, 1].set_title('recon_v2', fontsize=6)
    axs[1, 1].axis('off')

    axs[0, 0].imshow(plot_image(video1.squeeze(0), nrow=NUM_FRAMES, normalize=True))
    axs[0, 0].set_title('v1', fontsize=6)
    axs[0, 0].axis('off')

    axs[0, 1].imshow(plot_image(video2.squeeze(0), nrow=NUM_FRAMES, normalize=True))
    ...
    for j in range(NUM_FEA):
        ...
        axs[j + 1, 0].imshow(plot_image(v1_decoded_feature_transferred.squeeze(0), nrow=NUM_FRAMES, normalize=True))
        ...
        axs[j + 1, 1].imshow(plot_image(v2_decoded_feature_transferred.squeeze(0), nrow=NUM_FRAMES, normalize=True))
        axs[j + 1, 1].set_title('v2_feature' + str(j) + '_transferred', fontsize=6)
        axs[j + 1, 1].axis('off')

    plt.axis('off')
    plt.savefig(osp.join('outputs', 'style_transfer_training', DATASET, f'{epoch}_{flags.ENCODER_SAVE}.png'), dpi=1000)